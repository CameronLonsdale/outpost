				...
				if (group.expanded) {
					...
					for (int m=0; m<group.motions.Count; m++) {
						...
						if (used) {
							...
							GUI.changed = false;
							ma.animation = EditorGUILayout.ObjectField(ma.animation, typeof(AnimationClip), false) as AnimationClip;
							...
							EditorGUILayout.ObjectField(ma.animation, typeof(AnimationClip), false);
							GUI.enabled = true;
